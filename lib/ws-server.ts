...
// For backward compatibility, re-export subscribe from the manager
export function subscribe(topic: string, ws: WebSocket): void {
    serverManager?.subscribe(topic, ws)
}
...
export const api = {
    ...
    subscribe: (topic: string, handler: any, middlewares?: any[]) =>
        serverManager?.api.subscribe(topic, handler, middlewares),
}
